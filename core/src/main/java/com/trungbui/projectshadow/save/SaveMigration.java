...
public final class SaveMigration {
     ...
     *  v2 → v3 (post-Sprint-13 Fix E): added {@code heroDamageDealt} map. */
    public static final int CURRENT_RUN_VERSION = 3;
    ...
    public static RunState loadRun(ObjectMapper mapper, String json) throws IOException {
        ...
        // Migrate old saves to current version. Jackson will default missing fields
        // (ints → 0, Map → null, which the compact constructor normalises to empty).
        // We just bump the version field so the loaded record reports the current version.
